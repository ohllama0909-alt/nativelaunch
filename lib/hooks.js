...
export function useFleet() {
  ...
  useEffect(() => {
    ...
    api('/jobs')
      .then((res) => {
        if (cancelled) return;
        const list = res.jobs || [];
        const current = list.find((j) => j.status === 'running') || list[0] || null;
  ...
  useEffect(() => {
    ...
    const replace = (event) => {
      try {
        ...
        if (payload.activeJob !== undefined) setActiveJob(payload.activeJob);
        setLive(true);
      } catch (_) {
  ...
  return { bots, loading, live, error, activeJob, setActiveJob, reload, setBots };
}
  ...
  useEffect(() => {
    ...
    const onSnapshot = (event) => {
      try {
        ...
        if (payload.shards !== undefined) setShards(payload.shards);
        setLive(true);
      } catch (_) {}
    };
    ...
    const onShards = (event) => {
      try {
        setShards(JSON.parse(event.data).shards);
      } catch (_) {}
    };
    ...
    source.addEventListener('shards', onShards);
    source.onopen = () => setLive(true);
    source.onerror = () => setLive(false);
